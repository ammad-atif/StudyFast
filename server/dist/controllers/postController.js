...
exports.getComments = exports.addComment = exports.unsavePost = exports.savePost = exports.removeVote = exports.downvotePost = exports.upvotePost = exports.deletePost = exports.createPost = exports.updatePost = exports.getPostById = exports.getLibraryPosts = exports.getSubjectsAndTags = exports.getPosts = void 0;
const zod_1 = require("zod");
const mongoose_1 = __importDefault(require("mongoose"));
const Post_1 = __importDefault(require("../models/Post"));
const PostComment_1 = __importDefault(require("../models/PostComment"));
const PostVote_1 = __importDefault(require("../models/PostVote"));
...
const getPosts = async (req, res) => {
    try {
        const limit = 20;
        const parsed = postValidation_1.listPostsQueryRequestSchema.safeParse({
        ...
        if (!parsed.success) {
            return sendError(res, 400, "Invalid query parameters", "INVALID_QUERY", parsed.error);
        }
        ...
        const sortCriteria = sortBy === "most-upvoted"
        ...
        if (req.user && items.length > 0) {
            ...
            const userVotes = await PostVote_1.default.find({
                user: req.user._id,
                post: { $in: postIds },
            })
...
// @access  Public
const getSubjectsAndTags = async (_req, res) => {
    try {
        const [subjects, tags] = await Promise.all([
            Post_1.default.distinct("subject"),
            Post_1.default.distinct("tags"),
        ]);
        ...
        return sendError(res, 500, "Failed to fetch subjects and tags", "SERVER_ERROR");
    }
};
exports.getSubjectsAndTags = getSubjectsAndTags;
...
const getLibraryPosts = async (req, res) => {
    try {
        const user = ensureAuthUser(req, res);
        if (!user)
            return;
        const limit = 20;
        const libraryQuerySchema = zod_1.z
            .object({
        ...
        const [createdIdsRaw, likedIdsRaw, commentedIdsRaw] = await Promise.all([
            Post_1.default.find({ createdBy: user._id }).select("_id").lean(),
            PostVote_1.default.find({ user: user._id, voteType: "upvote" }).distinct("post"),
            PostComment_1.default.find({ user: user._id }).distinct("post"),
        ]);
        ...
        if (filter === "created") {
            ...
            filterIds = Array.from(new Set([...createdIds, ...likedIds, ...commentedIds, ...savedIds]));
        }
        ...
        const query = {
            _id: { $in: filterIds.map((id) => new mongoose_1.default.Types.ObjectId(id)) },
        };
        if (q) {
            query.$or = [
                { title: { $regex: q, $options: "i" } },
        ...
        const enrichedItems = items.map((post) => {
            ...
            return {
                ...
                viewer: {
                    isSaved: savedPostIds.has(postId),
                    userVote: voteByPostId.get(postId) ?? null,
                    isCreatedByViewer: post.createdBy._id.toString() === user._id.toString(),
                },
            };
        });
        return sendSuccess(res, 200, "Library posts fetched successfully", {
...
exports.getLibraryPosts = getLibraryPosts;
...
const getPostById = async (req, res) => {
    try {
        const parsed = postValidation_1.postIdParamsSchema.safeParse(req.params);
        if (!parsed.success) {
            return sendError(res, 400, "Invalid post id", "INVALID_POST_ID", parsed.error);
        }
        const post = await Post_1.default.findById(parsed.data.postId)
            .populate("createdBy", "fullName email")
            .lean();
        if (!post) {
        ...
        const [userVote] = await Promise.all([
            PostVote_1.default.findOne({ post: post._id, user: req.user._id })
                .select("voteType")
                .lean(),
        ]);
        const savedPostIds = new Set((req.user.savedPosts || []).map((id) => id.toString()));
...
const updatePost = async (req, res) => {
    try {
        ...
        if (!user)
            return;
        const paramsParsed = postValidation_1.postIdParamsSchema.safeParse(req.params);
        if (!paramsParsed.success) {
            return sendError(res, 400, "Invalid post id", "INVALID_POST_ID", paramsParsed.error);
        }
        const bodyParsed = postValidation_1.updatePostRequestSchema.safeParse(req.body);
        if (!bodyParsed.success) {
            return sendError(res, 400, "Invalid input data", "INVALID_INPUT", bodyParsed.error);
        }
        const post = await Post_1.default.findById(paramsParsed.data.postId);
        if (!post) {
            return sendError(res, 404, "Post not found", "POST_NOT_FOUND");
        }
        if (post.createdBy.toString() !== user._id.toString()) {
            return sendError(res, 403, "You can only edit your own posts", "FORBIDDEN");
        }
        ...
        await post.save();
        const updated = await Post_1.default.findById(post._id)
            .populate("createdBy", "fullName email")
            .lean();
...
const createPost = async (req, res) => {
    try {
        ...
        if (!user)
            return;
        const parsed = postValidation_1.createPostRequestSchema.safeParse(req.body);
        if (!parsed.success) {
        ...
        if (parsed.data.chatLink) {
            ...
            await Post_1.default.findByIdAndUpdate(post._id, {
                embeddingJobId: job.id,
                embeddingStatus: 'queued',
                embeddingUpdatedAt: new Date(),
            });
        }
        const populated = await Post_1.default.findById(post._id)
...
const deletePost = async (req, res) => {
    try {
        ...
        if (post.createdBy.toString() !== user._id.toString()) {
            ...
            return sendError(res, 403, "You can only delete your own posts", "FORBIDDEN");
        }
        await Promise.all([
            // Remove post and related records in parallel for better performance.
            Post_1.default.findByIdAndDelete(post._id),
            PostComment_1.default.deleteMany({ post: post._id }),
            PostVote_1.default.deleteMany({ post: post._id }),
            User_1.default.updateMany({}, { $pull: { savedPosts: post._id } }),
        ]);
...
const voteOnPost = async (req, res, voteType) => {
    // Example:
    ...
    const existingVote = await PostVote_1.default.findOne({
        post: post._id,
        user: user._id,
    });
    if (existingVote) {
        ...
        await PostVote_1.default.create({
            post: post._id,
            user: user._id,
            voteType,
        });
    ...
    const [upvotesCount, downvotesCount] = await Promise.all([
        PostVote_1.default.countDocuments({ post: post._id, voteType: "upvote" }),
        PostVote_1.default.countDocuments({ post: post._id, voteType: "downvote" }),
    ]);
...
const removeVote = async (req, res) => {
    try {
        ...
        await PostVote_1.default.deleteOne({ post: post._id, user: user._id });
        const [upvotesCount, downvotesCount] = await Promise.all([
            PostVote_1.default.countDocuments({ post: post._id, voteType: "upvote" }),
            PostVote_1.default.countDocuments({ post: post._id, voteType: "downvote" }),
        ]);
...
const savePost = async (req, res) => {
    try {
        ...
        const postExists = await Post_1.default.exists({ _id: parsed.data.postId });
        if (!postExists) {
            return sendError(res, 404, "Post not found", "POST_NOT_FOUND");
        }
        const alreadySaved = (user.savedPosts || []).some((id) => id.toString() === parsed.data.postId);
        if (!alreadySaved) {
            // Push post ID into user's saved list only once.
            user.savedPosts = [
                ...(user.savedPosts || []),
                new mongoose_1.default.Types.ObjectId(parsed.data.postId),
            ];
            await user.save();
        }
...
const unsavePost = async (req, res) => {
    try {
        ...
        user.savedPosts = (user.savedPosts || []).filter((id) => id.toString() !== parsed.data.postId);
        await user.save();
...
const addComment = async (req, res) => {
    try {
        ...
        const bodyParsed = postValidation_1.createCommentRequestSchema.safeParse(req.body);
        if (!bodyParsed.success) {
            return sendError(res, 400, "Invalid input data", "INVALID_INPUT", bodyParsed.error);
        }
        const post = await Post_1.default.findById(paramsParsed.data.postId);
        ...
        const created = await PostComment_1.default.create({
            post: post._id,
            user: user._id,
        ...
        await post.save();
        const comment = await PostComment_1.default.findById(created._id)
...
const getComments = async (req, res) => {
    try {
        ...
        const queryParsed = postValidation_1.listCommentsQueryRequestSchema.safeParse({
            order: normalizeQueryString(req.query.order),
            page: normalizeQueryString(req.query.page) ?? 1,
        });
        if (!queryParsed.success) {
            return sendError(res, 400, "Invalid query parameters", "INVALID_QUERY", queryParsed.error);
        }
        const postExists = await Post_1.default.exists({ _id: paramsParsed.data.postId });
        if (!postExists) {
            return sendError(res, 404, "Post not found", "POST_NOT_FOUND");
        }
        ...
        const [items, totalItems] = await Promise.all([
            PostComment_1.default.find({ post: paramsParsed.data.postId })
                .sort({ createdAt: sortOrder })
                .skip(skip)
                .limit(limit)
                .populate("user", "fullName avatar")
                .lean(),
            PostComment_1.default.countDocuments({ post: paramsParsed.data.postId }),
        ]);
